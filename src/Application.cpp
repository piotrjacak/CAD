        ...
        for (auto& obj : sceneObjects) 
        {
            ...
            else if (obj.type == objects::ObjectType::InterpolatingCurveC2)
            {
                if (obj.needsUpdate || obj.VAO == 0)
                {
                    ...
                    if (n >= 2)
                    {
                        // 1. Obliczenie odleg�o�ci (ci�ciw) mi�dzy w�z�ami
                        ...
                        // 2. Przygotowanie wektor�w dla macierzy tr�jdiagonalnej
                        std::vector<float> a(n, 0.0f); // Pod przek�tn�
                        std::vector<float> b(n, 0.0f); // G��wna przek�tna
                        std::vector<float> c(n, 0.0f); // Nad przek�tn�
                        std::vector<pmath::Vec3> r(n, pmath::Vec3(0.0f, 0.0f, 0.0f)); // Wektor wynik�w
                        ...
                        // R�wnania dla punkt�w wewn�trznych (i = 1 ... n-2)
                        for (int i = 1; i < n - 1; ++i)
                        {
                            a[i] = d[i];
                            b[i] = 2.0f * (d[i - 1] + d[i]);
                        ...
                        // 3. Algorytm Thomasa (Rozwi�zanie uk�adu tr�jdiagonalnego w czasie liniowym)
                        ...
                        // 4. Konwersja wektor�w stycznych na punkty kontrolne segment�w Beziera
                    ...
                    // 5. Przygotowanie bufor�w VBO/EBO dla GPU
                    if (obj.VAO == 0)
                    {
                        glGenVertexArrays(1, &obj.VAO);
                        glGenBuffers(1, &obj.VBO);
                ...
                // Opcjonalne rysowanie polilinii kontrolnej (pomara�czowej dla wgl�du)
                ...
                // Generowanie w�a�ciwej geometrii krzywej 
                if (obj.curveIndexCount > 0)
                {
                    bezierShader.use();
                    bezierShader.setMat4("model", curveModel);
